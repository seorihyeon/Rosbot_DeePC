...
class RandomCollectNode(CollectBase):
    ...
    def _load_random_parameters(self):
        self.random_dataset_steps = int(self.get_parameter('random_dataset_steps').value)
        ...
        self.random_cmd_dv_min = float(self.get_parameter('random_cmd_dv_min').value)
        self.random_cmd_dv_max = float(self.get_parameter('random_cmd_dv_max').value)
        self.random_cmd_dw_min = float(self.get_parameter('random_cmd_dw_min').value)
        self.random_cmd_dw_max = float(self.get_parameter('random_cmd_dw_max').value)
        self.random_hold_steps_min = int(self.get_parameter('random_hold_steps_min').value)
        self.random_hold_steps_max = int(self.get_parameter('random_hold_steps_max').value)
        self.random_min_command_norm = float(self.get_parameter('random_min_command_norm').value)

        if self.random_dataset_steps <= 0:
            raise ValueError("random_dataset_steps must be positive")
        if self.random_hold_steps_min <= 0 or self.random_hold_steps_max < self.random_hold_steps_min:
        ...
        if self.random_min_command_norm > max_norm + 1.0e-12:
            raise ValueError(
                "random_min_command_norm is too large for the configured random command ranges"
            )
    ...
    def log_progress(self, cmd_v: float, cmd_w: float, v_meas: float, w_meas: float, cmd_source: str) -> None:
    ...
    def sample_random_command(self):
        while True:
            ...
            if norm >= self.random_min_command_norm:
                self.current_cmd_v = clamp(self.current_cmd_v + dv, self.v_min, self.v_max)
                self.current_cmd_w = clamp(self.current_cmd_w + dw, self.w_min, self.w_max)
                break
