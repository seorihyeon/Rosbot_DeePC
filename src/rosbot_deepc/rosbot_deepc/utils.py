...
def unwrap_angle(new_angle: float, prev_angle: Optional[float], prev_unwrapped: Optional[float]) -> float:
    if prev_angle is None or prev_unwrapped is None:
        return new_angle

    delta = wrap_to_pi(new_angle - float(prev_angle))
...
def normalize_yaw_representation(value) -> str:
    ...
    if rep in VALID_YAW_REPRESENTATIONS:
        return rep
    raise ValueError(
        "yaw_representation must be one of: wrap, unwrap, trig"
    )
...
def yaw_representation_uses_trig(yaw_representation: str) -> bool:
    return normalize_yaw_representation(yaw_representation) == YAW_REPRESENTATION_TRIG

def encode_yaw_output(
    *,
    x: float,
    y: float,
    yaw: float,
    ...
    if rep == YAW_REPRESENTATION_TRIG:
        return [
            float(x),
            float(y),
            math.cos(float(yaw)),
    ...
    return [float(x), float(y), float(yaw), float(v), float(w)]

def yaw_prediction_fields(yaw_representation: str) -> List[str]:
    if normalize_yaw_representation(yaw_representation) == YAW_REPRESENTATION_TRIG:
        return ["y_cos_yaw", "y_sin_yaw"]
    return ["y_yaw"]
...
def _sanitize_w_meas_sequence(
    rows: List[dict],
    raw_yaws: List[float],
    *,
    max_abs_measured_w: float,
) -> List[float]:
    w_values = [float(row["w_meas"]) for row in rows]
    if max_abs_measured_w <= 0.0:
        return w_values

    times = [_optional_float(row, "sim_time_sec") for row in rows]
    last_valid: Optional[float] = None

    for idx, raw_w in enumerate(w_values):
        if math.isfinite(raw_w) and abs(raw_w) <= max_abs_measured_w:
            last_valid = raw_w
            continue

        replacement: Optional[float] = None
        if idx > 0 and times[idx] is not None and times[idx - 1] is not None:
            dt = float(times[idx]) - float(times[idx - 1])
            if dt > 0.0:
                candidate = wrap_to_pi(float(raw_yaws[idx]) - float(raw_yaws[idx - 1])) / dt
                if math.isfinite(candidate) and abs(candidate) <= max_abs_measured_w:
                    replacement = candidate

        if replacement is None:
            replacement = last_valid if last_valid is not None else 0.0

        w_values[idx] = replacement
        last_valid = replacement
...
def load_dataset_csv(
    ...
    max_rows: int = 0,
    sanitize_measured_w: bool = True,
    max_abs_measured_w: float = 10.0,
    ...
    required = ["cmd_v", "cmd_w", "x", "y", "yaw", "v_meas", "w_meas"]
    for key in required:
        if key not in rows[0]:
    ...
    w_meas_values = (
        _sanitize_w_meas_sequence(
            rows,
            raw_yaws,
            max_abs_measured_w=max_abs_measured_w,
        )
        if sanitize_measured_w
        else [float(row["w_meas"]) for row in rows]
    )
    ...
    for idx in range(aligned_len):
        ...
        w_meas = w_meas_values[idx + y_shift_steps]
        u_list.append([
            float(u_row["cmd_v"]),
            float(u_row["cmd_w"]),
        ])
    ...
    for path in csv_paths:
        try:
            u_data, y_data = load_dataset_csv(
                ...
                sanitize_measured_w=sanitize_measured_w,
                max_abs_measured_w=max_abs_measured_w,
                yaw_representation=yaw_representation,
...
def build_mosaic_input_hankel(datasets: List[dict], L: int) -> Tuple[np.ndarray, List[str]]:
...
def build_mosaic_hankel(datasets: list[dict], L: int) -> Tuple[np.ndarray, np.ndarray]:
    Hu_list: List[np.ndarray] = []; Hy_list: List[np.ndarray] = []
...
def numerical_rank(A: np.ndarray, tol: Optional[float] = None) -> Tuple[int, np.ndarray, float]:
...
def stack_history_with_zero_padding(hist: Deque[np.ndarray], dim: int, Tini: int) -> np.ndarray:
    seq: List[np.ndarray] = []
